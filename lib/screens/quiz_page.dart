...
class _QuizPageState extends State<QuizPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> loadQuiz() async {
    ...
    try {
      ...
      final result = await aiService.generateQuiz(
        widget.skillName,
        widget.topic,
        widget.currentHours,
        widget.totalHours,
        widget.resourceLinks,
      );
  ...
  void _submitAnswer(int index) {
    ...
    String feedback = isCorrect 
        ? "✅ Correct!" 
        : "❌ Incorrect. The correct answer was: ${questions[index]['answer']}";
  ...
  Future<void> submitQuiz() async {
    ...
    // Attempt to log practice if score is decent (e.g. at least 80%)
    final passThreshold = (questions.length * 0.8).ceil();
    if (score >= passThreshold && widget.practiceMinutes > 0) {
      try {
        await SkillService().logPractice(
          skillId: widget.skillId,
          minutes: widget.practiceMinutes,
       ...
       if (mounted) {
         ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Score too low (< 80%). Study more before practice time counts!"),
            backgroundColor: Colors.orange,
            behavior: SnackBarBehavior.floating,
          ),
        );
